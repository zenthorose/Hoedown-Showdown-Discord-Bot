...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const colorChoice = interaction.options.getString('color') || 'Gray'; // default to gray if not selected
    ...
    const embedColor = colorMap[colorChoice];
    ...
    if (description.trim().length > 0) {
      mainEmbed.setDescription(description.trim());
    } else {
      mainEmbed.setDescription('\u200B'); // prevent empty embed error
    }
